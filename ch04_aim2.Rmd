...
Constraining the coefficient of the FSMI Mate-seeking latent variable to 0 in a model that regressed number of sexual partners on this variable and gender did not result in significantly worse fit for either the unpartnered ($`r print_lrt_test_stats(fsmi_num_partners_lrt)`$) or full sample ($`r print_lrt_test_stats(fsmi_num_partners_all_lrt)`$).
The coefficient for both regressions were, as expected, positive.

Next we compared a model which added the three UPPS-P variables (Sensation Seeking, Positive and Negative Urgency) to a model which excluded them from the regression ordinal regression of the number of partners variable.
This resulted in significantly worse fit ($`r print_lrt_test_stats(uppsp_fsmi_partners_lrt, 2, 4)`$).
Fit was not significantly reduced by constraining the Mate-seeking coefficient to 0 in this model ($`r print_lrt_test_stats(uppsp_fsmi_partners_lrt2, 2, 4)`$).
The associations between the three UPPS-P variables and number of partners were all positive, whether or not FSMI Mate-seeking was included, which also had a positive coefficient.
