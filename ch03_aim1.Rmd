...
A Bayesian approach provides a robust framework for specifying and estimating hierarchical and nonlinear models with many parameters while constraining inferences in a way that reduces the rate of making incorrect claims with confidence [@gelman_why_2012;@GelmanPowerCalculationsAssessing2014].
...
Each individual's vector of $\theta_{j}$ is just the probability of pressing the right arrow key, $P(a_{\rightarrow t} | s_{t})_{t}$, for each trial $t$, conditional on whatever stimulus $s_{t}$ is present on that trial. 
...
This probability is then adjusted by a noise parameter, $0 \leq\xi_{jk}\leq1$ for each participant $j$ during condition $k$.
...
Associations between all model parameters, measures of performance, age, and pubertal development are computed using Spearman's rank order correlations and standardized coefficients from linear mixed effects models.
Linear mixed effects (LME) models were computed using the R package lme4 [version `r packageVersion('lme4')`; @bates2015].
These LME models were computed in order to aid descriptions of plotted data and best-fit generalized additive model (GAM) lines, and were used in place of correlations to account for non-independence of data when, for example, a parameter for all three conditions was being regressed on age.
All such models have the form $y_{ij} = \gamma_{00} + \gamma_{10}\text{x}_j + u_{0j} + e_{ij}$, where $y$ is a target outcome like the learning rate $\epsilon$, and $x$ is a predictor like age or score on the pubertal development scale.
...
The maximal model for each group included parameters (for each condition) for the learning rate ($\epsilon$), reward modifier (also referred to as inverse temperature; $\rho$), irreducible noise ($\xi$), and right-press bias ($b$).
...
\caption[Correlation between parameters in 2-level maximal model]{Correlation between parameters in 2-level maximal model. Points are poster means of individual parameter estimates. All parameters are shown on their untransformed (i.e., unconstrained) scales. Black lines are LOESS curves. The diagonal shows the density, while the upper triangle shows Spearman correlation coefficients.}
...
This is precisely what we seen in these data.
...
Comparing the proportion of optimal choices in the last half of the run, paired sample Yuen *t*-tests, which are robust to non-normality [@yuen1974], reject the null of no difference between the two conditions of interest and the control condition (Dating/Looking: `r apa_lowfi_ht_dl_t`; Popular/Unpopular: `r apa_lowfi_ht_pu_t`).
The differences in observed behavior are also reflected in some parameter estimates.
The 99.5% credible intervals for the difference in learning rate, $\epsilon$, between the two social conditions and the minimally social conditions are both well above 0, while the difference between the two social conditions has much of its mass close to 0 (Figure \ref{fig:epsdiff}).
This indicates that on average, individuals' learning rates in the social conditions are larger relative to the minimally social condition, but are not very different between them (although we might infer that the Popular/Unpopular learning rate is more likely to be of the greatest magnitude).
...
Finally, differences in right-bias, $b$, are also not convincingly either positive or negative, although there is a suggestion of greater tendency toward choosing the left social option (Dating, median $b = -0.11, \text{CI}_{.995} = [-0.20,-0.02]$; or Popular, median $b = -0.10, \text{CI}_{.995} = [-0.19,-0.01]$) than the left reference option (Thirsty, median $b = -0.05, \text{CI}_{.995} = [-0.12,0.034]$; Figure \ref{fig:bdiff}).
The credibly negative medians for each of the social conditions, as well as the suggestion of difference between the social and reference conditions, may indicate that participants favored choosing the socially desirable descriptors (Dating and Popular) for some reason.
...
Looking at the plot of 313 simulated runs in Figure \ref{fig:modpred} reveals a very slight effect of the bias toward choosing the left option (visible most easily at trial 0), as well as a more profound effect of parameter differences on both the magnitude of the actor's probability of choosing optimally, as well as the variance in that probability. 
...
With respect to the PDS, though there is considerably less variability in the college sample as compared to the adolescent sample, there is some variance, and so standardized coefficients are presented for the PDS for each subset as well.
...
Examining the relation between optimal choice improvement in social conditions and age reveals fairly flat associations (Figure \ref{fig:poptdiffpdsplot}).
...
For both the reward magnitude, $\rho$, and irreducible noise, $\xi$, parameters show age relations that are consistent with what we see above regarding the relations of both parameters and age to performance. 
...
\caption[Reinforcement learning model parameters versus age]{Reinforcement learning model parameters versus age. All best-fit lines are from generalized additive models. Grey line is fit to all data. Parameters are plotted on the unconstrained scales with possible values in $(-\infty, \infty)$.}
\label{fig:ageparplots}
\end{figure}

The difference between the social conditions and minimally social reference condition across this developmental span is one of the primary observations that will inform the question about whether motive effects on learning reflect what would be expected based on the theories discussed in the introduction.
...
The standardized coefficients, nearly all very small, mirror the best fit lines, although the estimate for $\epsilon$ in the adolescent sample is small and negative (Table \ref{tab:agebetas}).
...
\caption[Reinforcement learning model parameter condition contrasts versus age]{Reinforcement learning model parameter condition contrasts versus age. All best-fit lines are from generalized additive models. Grey line is fit to all data. Differences are culculated using paramters transformed to be on the model scale.}
...
\begin{tablenotes}
\footnotesize
\item Models were estimated using posterior mean parameters on unconstrained scales.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\begin{tablenotes}
\footnotesize
\item Parameter contrasts were computed by subtracting the mean posterior parameter (on the constrained scales) in the Hungry/Thirsty condition from parameter estimates in the two social conditions.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\caption[Reinforcement learning model parameter condition contrasts versus puberty]{Reinforcement learning model parameter condition contrasts versus puberty. All best-fit lines are from generalized additive models. Grey line is fit to all data. Differences are calculated using paramters transformed to be on the model scale. PDS = Pubertal Development Scale.}
...
That participants may have been sensitive to variation along this dimension is speculative, but this is at least one influence that is not related to social motivations that could result in salience differences between descriptor pairs and so explain the behavior of participants on this task.
...
These two robust studies are both consistent with improvements in performance on reinforcement learning tasks across adolescence.
...
In one study, adolescents performed better than adults, which was interpreted as indicated that heightened reward sensitivity may lead to better learning during adolescence [@davidow2016].
Like many of the studies which show greatest performance during adulthood, -@davidow2016 used a probabilistic reinforcement learning task.
Notably, adults showed higher learning rate parameters than adolescents (but performed worse as a result).
The results of their study are likely anomalous given the considerable evidence for greater adult performance on standard reinforcement learning paradigms.
...
The only study in which this relation is reversed was [@davidow2016], though another study showed differently signed relations between learning rates for positive versus negative feedback [@christakou2013].
...
Though the author present a several non-significant _p_-values to support the equivalence of the adolescent and adult preference profiles, the sample size is quite under-powered to detect differences, and it is not clear what size differences might matter with respect to learning and prediction error differences across age-groups.
